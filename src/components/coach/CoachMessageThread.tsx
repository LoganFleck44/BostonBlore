...
export function CoachMessageThread({ messages: initial, toId }: { messages: Msg[]; toId: string }) {
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden rounded-2xl border border-ink-600 bg-ink-700">
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && <p className="text-center text-sm text-ash mt-8">No messages yet.</p>}
        {messages.map((m) => (
      ...
      <div className="shrink-0 border-t border-ink-600 p-3 flex gap-2">
        <textarea value={body} onChange={(e) => setBody(e.target.value)}
          ...
          rows={1} placeholder="Message client…"
